// C:\laragon\www\sgcpro\src\public\script\react_modelo_v1\frontend\src\services\telefone.js
...
// Serviço para gerenciar telefones
const TelefoneService = {

    getEndPoint: async () => {
        const url = '/telefone/endpoint/salvar';
        let gov_br = [];
        let token_csrf = '';
        let getURI = [];
        try {
    ...
    getAll: async (page = 1, limit = 10) => {
        ...
        const url = `/telefone/api/exibir?page=${page}&limit=${limit}`;
    ...
    getById: async (id) => {
        const url = `/telefone/api/exibir/${id}`;
    ...
    postFilter: async (data, page = 1, limit = 10) => {
        console.log('postFilter/data :: ', data);

        const url = `/telefone/api/filtrar?page=${page}&limit=${limit}`;
        // console.log('url (C:/laragon/www/sgcpro/src/public/script/react_modelo_v1/frontend/src/services/telefone.js):', url);

        try {
            const response = await api.post(url, data);
            if (response.data.result.dbResponse !== undefined) {
                // console.log('postFilter/response :: ', response.data.result.dbResponse);
                return response.data.result.dbResponse;
            } else {
                return [];
            }
